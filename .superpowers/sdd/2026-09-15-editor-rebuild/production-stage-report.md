...
## Actual outline rendition preview

The toolbar now selects `Live HTML` (default) or `Fixed-copy outlines`. Outline mode POSTs its mode as part of render identity and uses the actual production outline export pipeline, freezing the supplied effective row as the sole Default row. It displays real SVG paths and seeks their actual production CSS animations. Last-ready double buffering remains, but pending mode changes cannot publish or interact as the new rendition.

Outline-mode exports do not snapshot SVG hosts as if they were font text. They wait for the current outlined stage to be ready, return no client text snapshots, and use the server's font-browser capture for the frozen row. SVG font/spacing telemetry reads the metadata added by parent to the actual bake result. Opening standalone View HTML carries the chosen preview mode.

Observed the new outline-view endpoint test fail on missing `outlined-text` before implementation; it now passes with actual paths, triple state and no live feed bridge. Route + store scoped tests: 16 passed. Browser script `output/playwright/verify-outline-rendition.mjs` chose the nondefault triple row, selected outline preview, sought 37.5%, downloaded the actual six-format outlined HTML package, and compared all 16 outlined text targets in 300x600: complete SVG markup and left/top/width/height matched exactly. Posted client `presentationSnapshots` was empty as required; the payload contained one triple Default row. Switching back to Live HTML succeeded. TypeScript showed no errors in these files, and scoped whitespace checks passed. These changes are left unstaged for the parent's integration commit.

## Export keeps the delivered rendition visible

The HTML/client/base export methods now call `prepareExportPreview(renderMode)` before capturing or downloading. It selects the requested rendition, invalidates the old-mode stage immediately, waits for matching size **and mode**, and verifies source identity. The selected rendition remains visible after export; font export switches back from outlines. Bulk Sync Zips retains its existing capture transaction.

A readiness regression first returned the old same-size font stage for an outline wait; optional mode-specific readiness fixes that. Store tests exercise the preparation call on all three fixed-copy methods. Scoped bridge/store tests: 20 pass. The actual six-format active-triple browser download test now starts from Live HTML, verifies ready outlines remain selected after outline export, then downloads font HTML and verifies ready Live HTML is selected with all feed rows retained. Changes remain unstaged/uncommitted for parent integration.

## Exact source readiness

Published stages now carry the raw document and effective-row object references that produced their HTML. Optional expected-source readiness requires both references as well as format/rendition; registry invalidation clears the metadata. Export preparation and font snapshot walking supply those references, closing the immediate same-mode edit/export race before React effects invalidate the old frame. Existing callers without expected source retain their prior contract.

The regression first accepted document A's same-size/same-mode frame while waiting for B, then passed after source matching. A second readiness test accepts intentional empty `src` attributes while rejecting failed nonempty authored image URLs. Bridge/store scoped tests: 24 passed, relevant TypeScript output clean and scoped diff whitespace clean. No staging or commit performed.
