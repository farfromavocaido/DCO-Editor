...
Offers-0 layers: `bg-blur` (`kind: "blur"`, `src/lib/blur-layer.ts`; stays hidden). Bluewave begins at stage1, moves to the exact prior committed settle (stage2) while green fades in, then returns to stage1 at the end (no opacity fade). Greenwave fades in at rest ~0.5s before Act 4 via offer-scoped clips. White logo (`logo-act3`) fades in from mid-open over 5% and out at `end`. Blue logo (`logo-act1`) stays hidden except **320×50**, which is navy-on-green for the whole offers-0 (photo/blue/scrim hidden, green held, navy type + logo) and uses that same hold-to-end fade on every ink, CTA, and roundel combination. T&C lines enter with offers-0 Act 2 (`offers0_act2_in`, the equal-split second photo act) and fade out at `green_in` (banner frames-4 still at `roundel_in`). Other sizes keep T&C ink white. Photo headlines keep offers-0 box geometry (white/navy ink); Act 4 + CTA are navy over green. Offers-0 CTA and Offer Roundel (frame / copy / value) each have their own `offers-0` / `offers-0.*` rules so rearranging either stack does not move offers 1–3. `headline-scrim` (`kind: "gradient"`, `src/lib/gradient-layer.ts`) — bottom-up (`to-top`), under the blue wave; always visible under `offers-0` (supports white T&Cs).
