...
- [ ] Write tests for production HTML response, latest-request ownership, target ID mapping and exact CSS-animation seeking.
- [ ] Run the tests and establish the missing behaviour.
- [ ] Host exported HTML; map rendered targets to outer selection/drag/resize controls; preserve multi-selection and stage zoom. Handle readiness and errors without stale stage replacement.
- [ ] Verify edits, feed state switching, seeking, undo and snapshot capture in a real browser.
- [ ] Commit and report exact tests and remaining integration constraints.
...
- [ ] Write behavioural tests that edit MPU zero-offer maxLines while preserving baseline and unrelated variants, test compound scopes and unscoped rules, and assert validation does not mutate documents.
- [ ] Run failing tests before changes.
- [ ] Centralise property/fit resolution and source selection. Expose active/shared edit intent and per-field provenance; remove silent roundel deletion from validation and replace migration with an explicit operation.
- [ ] Implement deliberate local reset, shared edit and detach semantics without relying on CSS class equality as ownership. Keep compatibility with existing campaigns.
- [ ] Verify save/reload and undo preserve active values and relations; commit and report.
...
- [ ] Test fixed frame preservation, whole-text multiline fit, impossible-fit diagnostics, stable shared fitting and parameterised motion distances.
- [ ] Observe failures, then implement policy-driven fitting and explicit relative-distance resolution while preserving legacy defaults for unmigrated documents.
- [ ] Add authorable controls via a small component integrated with Task 2; avoid hardcoded per-format design decisions.
- [ ] Verify short/long copy and relative distances across representative formats; commit and report.
...
- [ ] Replace the two baseline creative-assumption failures with fixtures/assertions of supported behaviour.
- [ ] Exercise editor/packaged parity, active fit editing, scope independence, drag, resize, undo, save/reload and campaign switching in the browser.
- [ ] Run npm test, npm run build, git diff --check; record limitations and migration evidence honestly.
- [ ] Review the complete diff and address regressions. Commit the verified milestone; preserve the worktree for user review.
