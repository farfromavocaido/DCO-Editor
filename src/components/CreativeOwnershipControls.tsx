// @ts-nocheck
'use client';
import {componentSourceLinks,componentLinkForTarget} from '@/lib/creative-components';
...
export function CreativeOwnershipControls({document,size,target,scopes}) {
  ...
  return <section className="inspector-section ownership-controls"><h3>Copy properties</h3><div className="ownership-body">
    ...
    {!memberships.length&&!positionSharing.length&&!componentSourceLinks(document,size,target.id,scopes).length&&!componentLinkForTarget(document,size,target.id,scopes) && <p className="inspector-note">No named links or inherited position sharing.</p>}
