...
type Rule = {
  id: string; name: string; type: 'conditional' | 'spacing'; enabled: boolean;
  targets: { size: string; targetId: string; scope?: string }[];
  when?: string[]; values?: Record<string, string | number>;
...
type Diagnostic = { id: string; targetId: string; size: string; status: 'disabled' | 'inactive' | 'active' | 'missing' | 'error'; message?: string };
...
export function LayoutRulesPanel({ document, size, targetId, scopes, diagnostics = [], onChange, onSelectRule, onFreeze }: Props) {
  ...
  const applicable = rules.filter(rule => rule.targets.some(target => target.size === size && target.targetId === targetId));
  ...
  const start = (type: Rule['type']) => {
    setError('');
    setDraft({ id: newId(), name: type === 'spacing' ? 'Responsive spacing' : 'Conditional placement', type, enabled: true, targets: [{ size, targetId }], when: [],
  ...
  const conditionText = (rule: Rule) => (rule.when || []).map(scope => {
  ...
  return <div className={styles.panel}>
    ...
    {!draft && <div className={styles.actions}><button type="button" onClick={() => start('conditional')}>+ Conditional placement</button><button type="button" onClick={() => start('spacing')}>+ Responsive spacing</button></div>}
    {draft && <form className={styles.draft} onSubmit={event => {
      event.preventDefault();
      if (draft.type === 'conditional' && !Object.keys(draft.values || {}).length) { setError('Choose at least one placement value.'); return; }
      ...
      <strong>{rules.some(rule => rule.id === draft.id) ? 'Edit' : 'New'} {draft.type === 'spacing' ? 'responsive spacing' : 'conditional placement'}</strong>
      ...
      {draft.type === 'conditional' ? <fieldset><legend>Placement overrides</legend><p className={styles.note}>Leave a field blank to keep its authored value.</p><div className={styles.grid}>{numericFields.map(field => <label key={field} className={styles.field}><span>{field[0].toUpperCase() + field.slice(1)} (px)</span><input type="number" step="any" value={draft.values?.[field] ?? ''} onChange={event => { const values = { ...draft.values }; if (event.target.value === '') delete values[field]; else values[field] = Number(event.target.value); patch({ values }); }} /></label>)}</div>
        ...
        <label className={styles.field}><span>This element’s ink edge</span><select aria-label="This element’s ink edge" value={draft.targetEdge} onChange={event => patch({ targetEdge: event.target.value as Edge })}>{(['start', 'center', 'end'] as Edge[]).map(edge => <option key={edge} value={edge}>{edgeLabel(edge, draft.axis)}</option>)}</select></label>
        <label className={styles.field}><span>Reference</span><select aria-label="Reference" value={draft.reference?.targetId} onChange={event => patch({ reference: { targetId: event.target.value, edge: draft.reference?.edge || 'start' } })}><option value="canvas">Canvas</option>{targets.filter(target => target.id !== targetId).map(target => <option key={target.id} value={target.id}>{target.label}</option>)}</select></label>
        ...
        <div className={styles.grid}><label className={styles.field}><span>Gap</span><input required type="number" step="any" value={draft.gap ?? 0} onChange={event => patch({ gap: Number(event.target.value) })} /></label><label className={styles.field}><span>Units</span><select aria-label="Units" value={draft.gapUnit || 'px'} onChange={event => patch({ gapUnit: event.target.value as Rule['gapUnit'] })}><option value="px">Pixels</option><option value="em">Target font size (em)</option><option value="percent">Canvas axis (%)</option></select></label></div>
        <p className={styles.note}>Positive gaps move right or down; negative gaps move left or up. Visible letter shapes and image alpha define the ink edge.</p>
