...
describe('relationship transactions', () => {
 it('closes after explicit apply, offers undo, and resets selections on reopening', async () => {
  ...
  await click(button('Copy appearance…'));
  expect(document.querySelector('[role=dialog]')).not.toBeNull();
  const destination=[...document.querySelectorAll('.visual-copy-card-select')].find(item=>item.getAttribute('aria-label').includes('Business'));
  await click(destination);
  await click(button('Typography'));
  expect(state.applyCreativeOwnershipDocument).not.toHaveBeenCalled();
  expect(button('Copy typography to 1 ad').disabled).toBe(false);
  await click(button('Before')); await click(button('After'));
  expect(state.applyCreativeOwnershipDocument).not.toHaveBeenCalled();
  await click(button('Copy typography to 1 ad'));
  expect(state.applyCreativeOwnershipDocument).toHaveBeenCalledOnce();
  expect(document.querySelector('[role=dialog]')).toBeNull();
  expect(button('Undo')).toBeDefined();
  await click(button('Copy appearance…'));
  expect(button('Copy appearance to 0 ads').disabled).toBe(true);
  await click(button('×'));
 ...
 it('cancel leaves document untouched and linked controls are separate', async () => {
  ...
  expect(button('Link appearance to 0 ads').disabled).toBe(true);
  await click(button('×'));
  expect(state.applyCreativeOwnershipDocument).not.toHaveBeenCalled();
 });
});
