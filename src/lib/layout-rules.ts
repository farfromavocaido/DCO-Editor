...
export function layoutRulesForSize(document,size){validateLayoutRules(document);return getLayoutRules(document).flatMap(rule=>rule.type==='distribute'?(rule.targets.some(t=>t.size===size)?[{...rule,size,transitionCases:transitionCases(document,size,rule),area:rule.areas[size],members:rule.targets.filter(t=>t.size===size).map(t=>t.targetId),textMembers:rule.targets.filter(t=>t.size===size&&(t.targetId.includes('::')||document.sizes[size].layers.find(l=>l.id===t.targetId)?.kind==='text')).map(t=>t.targetId),targetId:rule.targets.find(t=>t.size===size).targetId,scope:'',targets:undefined}]:[]):rule.targets.filter(t=>t.size===size).map(target=>({...rule,targetId:target.targetId,scope:target.scope||'',size,targets:undefined})));}
