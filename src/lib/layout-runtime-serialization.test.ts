...
test('Pages/tsx emits a self-contained layout sequence which executes without build helpers',()=>{
 const source=execFileSync(process.execPath,[require.resolve('tsx/cli'),'--tsconfig','tsconfig.json','src/test/fixtures/emit-layout-sequence.ts'],{cwd:process.cwd(),encoding:'utf8'});
