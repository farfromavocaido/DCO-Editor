...
export function renderedGeometry(targetId){const stage=getProductionStage();if(!stage)return null;const runtime=createResponsiveLayoutRuntime(stage.ownerDocument.defaultView),element=runtime.element(targetId);if(!element)return null;const style=stage.ownerDocument.defaultView.getComputedStyle(element);const result=Object.fromEntries(['left','top','width','height'].map(key=>[key,parseFloat(style[key])||0]));if(stage.ownerDocument.defaultView.__DCO_LAYOUT_TRANSITIONS__?.some(p=>p.targetId===targetId)){const translate=String(style.translate||'').split(' ');result.left+=parseFloat(translate[0])||0;result.top+=parseFloat(translate[1])||0;result.layoutMotion=true;}return result;}
export function captureTextAnchor(targetId,edge='end'){
 const stage=getProductionStage();if(!stage)throw new Error('Wait for the preview before pinning text');
