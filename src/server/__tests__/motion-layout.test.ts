...
beforeAll(async()=>{browser=await chromium.launch();const fontUrlMap={};for(const face of campaignFontFaces(sse)){const bytes=await fs.readFile(`${projectRoot}/assets/${face.asset}`);fontUrlMap[face.asset.split('/').pop()]=`data:font/otf;base64,${bytes.toString('base64')}`;}fontOptions={fontUrlMap};});
...
test('a visible reset is rejected but a loop return is optional',()=>{const d=fixture();d.layoutRules=[area()];d.layoutRules[0].transition.return={mode:'hidden',startS:8};expect(()=>validateLayoutRules(d)).toThrow(/visible/);d.layoutRules[0].transition.return={mode:'none'};expect(()=>validateLayoutRules(d)).not.toThrow();});
