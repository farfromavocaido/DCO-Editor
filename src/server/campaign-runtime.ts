...
export const campaignRuntimeScript = (document, fitRules, options = {}) => {const usesLayout=Boolean(options.layoutRules?.length)||JSON.stringify(fitRules).includes('"anchor":');return `
${usesLayout ? `<script type="application/json" id="dco-layout-rules">${json(options.layoutRules||[])}</script>` : ''}
...
<script>(function(){
  ...
  function fit(){if(root){${usesLayout ? 'responsiveLayout.reset();' : ''}engine.applyRules(root,rules);${usesLayout ? 'responsiveLayout.anchorText(root,rules,engine.resolveRule);responsiveLayout.run(root,layoutRules,function(){engine.applyRules(root,rules);responsiveLayout.anchorText(root,rules,engine.resolveRule);});' : ''}}}
  function apply(row){
    ...
    window.__SSE_DCO_SETTLED__ = Promise.all([document.fonts && document.fonts.ready${usesLayout ? `,...Array.from(document.images).map(function(img){return img.complete?Promise.resolve():new Promise(function(resolve){img.addEventListener('load',resolve,{once:true});img.addEventListener('error',resolve,{once:true});});})` : ''}]).then(function(){return new Promise(function(resolve){
