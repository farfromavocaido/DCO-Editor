...
<script>(function(){
  var dimensions = ${json(campaignVariantModel(document).dimensions)};
  ...
  function fit(){if(root) engine.applyRules(root,rules);}
  function apply(row){
    root = document.getElementById('page-content'); if(!root) return;
    currentRow = row || {}; window.__SSE_DCO_APPLIED_ROW__ = currentRow;
    ...
    window.__SSE_DCO_SETTLED__ = Promise.resolve(document.fonts && document.fonts.ready).then(function(){return new Promise(function(resolve){
